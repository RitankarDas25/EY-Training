...
while True:
    ...
    if user_input.lower().startswith("summarize"):
        try:
            ...
            if not text_to_summarize:
                print(
                    "Agent: Please provide text to summarize. Example: summarize The meeting discussed cloud migration...")
                continue
    ...
    if user_input.lower().startswith("note"):
        try:
            ...
            if not note_text:
                print(
                    "Agent: Please provide a note to remember. Example: note Remember to email the project report tomorrow.")
                continue
    ...
    if user_input.lower() == "get notes":
        try:
            ...
            memory.save_context({"input": user_input}, {
                "output": f"You currently have {len(notes)} note(s): {note_list}" if notes else "You have no notes stored."})
            continue
        except Exception as e:
    ...
    if user_input.lower().startswith("improve"):
        try:
            ...
            if not text_to_improve:
                print(
                    "Agent: Please provide text to improve. Example: improve This report is kind of messy and confusing.")
                continue
